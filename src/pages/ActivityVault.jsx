        ...
        <div key={di} style={{display:'flex',alignItems:'center',gap:1,marginBottom:1}}>
          ...
          {row.map((v,hi)=>(
            <div key={hi} onMouseEnter={()=>setTip({d:di,h:hi,v})} onMouseLeave={()=>setTip(null)}
              style={{flex:'1 1 0',height:14,background:intensity(v),borderRadius:2.5,cursor:'crosshair',
...
export default function ActivityVault() {
  ...
  useEffect(()=>{
    const data=Array.from({length:80},(_,i)=>makeActivity(i));
    setActivities(data);
  ...
  useEffect(()=>{
    ...
    const iv=setInterval(()=>{
      ...
      setActivities(p=>{const n=[a,...p];if(n.length>120)n.pop();return n;});
    },6000);
